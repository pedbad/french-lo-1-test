...
export const OriginsAndLanguagesPronunciationSilentEnt = memo(function OriginsAndLanguagesPronunciationSilentEnt({ id }) {
  return (
    <div className={`lo3-demystify1-container container`} id={id || undefined}>
      <div className={`panel pronunciation-panel`} id={id ? `${id}Panel1` : undefined}>
        {/* The -ent ending is SILENT — there is no sound to demonstrate in the h3,
					    so the heading stays as plain text (same rationale as LO1 tab 4 "silent h"). */}
...
export const OriginsAndLanguagesPronunciationFinalConsonantsLiaison = memo(function OriginsAndLanguagesPronunciationFinalConsonantsLiaison({ id }) {
  return (
    <div className={`lo3-demystify2-container container`} id={id || undefined}>
      <div className={`panel pronunciation-panel`} id={id ? `${id}Panel2` : undefined}>
        {/* Liaison is a contextual phenomenon requiring contrasting pairs to explain;
					    no single word represents the topic, so the h3 stays as plain text. */}
...
export const OriginsAndLanguagesPronunciationEhSound = memo(function OriginsAndLanguagesPronunciationEhSound({ id }) {
  return (
    <div className={`lo3-demystify3-container container`} id={id || undefined}>
      <div className={`panel pronunciation-panel`} id={id ? `${id}Panel3` : undefined}>
        <h3>3. The sound &quot;<AudioClip className={`link`} soundFile={`audio/lo3/pronunciation/demystify/016-et.mp3`}>eh</AudioClip>&quot;</h3>
        <div className={`pronunciation-content`}>
          <div className={`pronunciation-text`}>
