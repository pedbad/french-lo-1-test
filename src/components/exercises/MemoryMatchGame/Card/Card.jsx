	...
	render = () => {
		...
		return (
			<div
				...
				className={cn(
					"memory-card group relative aspect-square w-full cursor-pointer select-none overflow-hidden rounded-xl border p-0 text-center font-bold text-card-foreground [perspective:1200px] transition-[transform,box-shadow,border-color,background-color] duration-300 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
					tiltClass,
					stateClass,
					className
				)}
			...
			>
				<div
					className={cn(
						"relative h-full w-full rounded-[inherit] [transform-style:preserve-3d] transition-transform ease-[cubic-bezier(0.22,0.9,0.25,1)]",
						flipClass
					)}
				...
				>
					<div
						aria-hidden="true"
						className="absolute inset-0 h-full w-full overflow-hidden rounded-[inherit] bg-[linear-gradient(160deg,rgb(var(--color-primary-50)_/_0.98),rgb(var(--color-primary-200)_/_0.92))] text-[rgb(var(--color-text-secondary)_/_1)] shadow-[inset_0_1px_0_rgb(var(--color-primary-50)_/_0.85),inset_0_-12px_18px_rgb(var(--color-primary-300)_/_0.16)] [backface-visibility:hidden]"
					>
						...
						<div className="absolute inset-0 flex items-center justify-center">
							<span className="relative z-[1] block text-[1.35rem] font-bold leading-none">?</span>
						</div>
					</div>
					<div className="absolute inset-0 flex h-full w-full rounded-[inherit] bg-[linear-gradient(180deg,rgb(var(--color-background)_/_1),rgb(var(--color-primary-50)_/_0.42))] p-2 [backface-visibility:hidden] [transform:rotateY(180deg)]">
