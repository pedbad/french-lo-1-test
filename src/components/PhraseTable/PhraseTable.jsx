...
// Strip HTML tags, lowercase (fr), and remove accents for sort comparison.
...
const isAudioPath = (value) =>
  typeof value === "string" && /\.mp3(?:[?#].*)?$/i.test(value.trim());

const getRowSortKey = (row = []) => {
  ...
  for (const cell of row) {
    ...
    if (!trimmed) continue;
    if (isAudioPath(trimmed)) continue;
    return normalizeForSort(trimmed);
  }
  return "";
};
...
export function PhraseTable({ config = {}, targetLanguageCode }) {
  const [tableSort, setTableSort] = useState("natural");

  // Detached copy of the phrases so we never mutate config.phrases.
  const originalPhrases = useMemo(
    () => ((config && config.phrases) || []).map((row) => [...row]),
    [config]
  );
  ...
  const basePhrases = originalPhrases || [];
  ...
  if (tableSort === "natural") {
    ...
    nonBlank.sort((a, b) => {
      const A = getRowSortKey(a);
      const B = getRowSortKey(b);
    ...
    if (phrase[0] === "" && phrase.length === 1) {
      ...
      rows.push(
        <TableRow
          ...
          key={`row${i}`}
          onClick={(e) => handleRowClick(rowSoundFile, e)}
        >
          {cells}
        </TableRow>
      );
              ...
              <TooltipTrigger asChild>
                <span className="inline-flex">
                  <IconButton
                    ...
                    size="lg"
                    variant="default"
                    onClick={() => setTableSort("alphabetical")}
                    ariaLabel={`Vocabulary organised alphabetically`}
                  >
                    Alphabetical
                  </IconButton>
