...
//
// Config is read-only here (id/items/cheatText/htmlContent come straight from
// props). The original class spread config into state once at construct and had
// no componentDidUpdate, so config never reset on prop change — deriving the
// read-only keys from props preserves that. Only the runtime counters
// (complete/failCount/nPlaced) live in state.
export function WordSpotExercise({ config = {}, suppressInfo = false }) {
  const {
    cheatText,
    htmlContent,
    id = [],
  ...
  const [nPlaced, setNPlaced] = useState(0);

  // Assigned during render (below), read by handlers — same pattern as the
  // class instance field `this.nToSolve`.
  const nToSolveRef = useRef(0);

  const autoSolve = () => {
    if (nPlaced < nToSolveRef.current) {
      const targets = document.querySelectorAll(`#${id} span.target`);
      for (let i = 0; i < targets.length; i++) {
        targets[i].classList.add('animate');
      }
      setComplete(true);
      setNPlaced(nToSolveRef.current);
    }
  };

  const handlePartWordClick = (e) => {
    if (e.target.classList.contains("animate") || e.target.classList.contains("error")) return;
    e.target.classList.add("animate");
    const next = nPlaced + 1;
    if (next === nToSolveRef.current) {
      setComplete(true);
    }
    setNPlaced(next);
  };

  const handlePartWordError = (e) => {
