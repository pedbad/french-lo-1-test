...
const SELECT_EXERCISE_TRIGGER_CLASS = "w-full min-h-10 text-[length:var(--font-size-sm)] md:text-base";
const SELECT_EXERCISE_INLINE_TRIGGER_CLASS = "inline-flex min-h-9 w-auto max-w-full align-middle text-[length:var(--font-size-sm)] md:text-base";
const SELECT_EXERCISE_INLINE_PASSAGE_TRIGGER_CLASS = "inline-flex h-8 w-auto max-w-full px-2 align-middle text-[length:var(--font-size-sm)] leading-[var(--line-height-app)]";
				...
				>
					...
					<SelectContent>
						{meta.options.map((option, optionIndex) => (
							<SelectItem
								className="text-[length:var(--font-size-sm)] md:text-base"
			...
			const rowHasChoices = rowBlankIndices.length > 0;
				...
				if (layoutMode === "inline-passage") {
					...
					passageLines.push(
						...
						<div
							className={`relative m-0 overflow-hidden text-[length:var(--font-size-sm)] leading-[var(--line-height-app)] md:text-base ${
								isPassageMeta
			...
			rows.push(
				...
				>
					...
					>
						...
						{renderInlineChoices ? (
							<div className="col-start-2 min-w-0 text-[length:var(--font-size-sm)] leading-[var(--line-height-app)] md:text-base">
								{segments.map((segment, segmentIndex) => {
									if (segment.type !== "choice") {
										return (
											<React.Fragment key={segment.key || `seg-${segmentIndex}`}>
							...
							<>
								{item.audio ? (
									...
									<div className="col-start-2 row-start-1 m-0 min-w-0 text-[length:var(--font-size-sm)] leading-[var(--line-height-app)] md:text-base">
										{this.renderSentenceWithoutChoices(segments)}
									</div>
								)}
