 ...
 *
 * Audio strategy: uses real stable clips from LO1/LO7 for components that
 * need audio. Text-only components (DraggableFillGaps, LineMatch, etc.) omit
 * audio to keep the fixture self-contained. All paths resolve via resolveAsset.
 *
...
const SELECT_CONFIG = {
  ...
  items: [
    {
      text: 'Je [*vais|vas|va|allons] à Paris.',
      audio: 'audio/lo1/exercises/01-word-parts/001-bonjour-mon-cheri.mp3',
    },
    {
      text: 'Tu [vais|*vas|va|allons] au cinéma ?',
      audio: 'audio/lo1/exercises/01-word-parts/002-bonsoir-mon-amour.mp3',
    },
    {
      text: "Elle [vais|vas|*va|allons] à l'école.",
      audio: 'audio/lo1/exercises/01-word-parts/003-au-revoir-ma-cherie.mp3',
    },
  ],
...
const WORD_ORDER_CONFIG = {
  ...
  soundFile: 'audio/lo1/exercises/05-listening-order/001-listening-order-1.mp3',
...
const RADIO_QUIZ_CONFIG = {
  ...
  options: ['Vrai', 'Faux'],
  soundFile: 'audio/lo1/exercises/07-listening-order/001-homme.mp3',
  phrases: [
...
const LINE_MATCH_CONFIG = {
  ...
  informationText: 'Match each French word to its English translation.',
  cheatText: 'Show answer',
  sampleSize: 4,
  items: [
    { id: 'bonjour', label: 'bonjour', audio: 'audio/lo1/dialogues/01/001-salut-ca-va.mp3' },
    { id: 'merci', label: 'merci', audio: 'audio/lo1/dialogues/02/003-ca-va-merci.mp3' },
    { id: 'au-revoir', label: 'au revoir', audio: 'audio/lo1/exercises/01-word-parts/003-au-revoir-ma-cherie.mp3' },
    { id: 'sil-vous-plait', label: "s'il vous plaît", audio: 'audio/lo1/exercises/01-word-parts/001-bonjour-mon-cheri.mp3' },
    { id: 'excusez-moi', label: 'excusez-moi', audio: 'audio/lo1/exercises/01-word-parts/002-bonsoir-mon-amour.mp3' },
    { id: 'oui', label: 'oui', audio: 'audio/lo1/exercises/01-word-parts/004-quelle-horreur-valerie.mp3' },
  ],
  titleText: 'LineMatch',
};
...
const MEMORY_MATCH_CONFIG = {
  ...
  cards: [
    {
      foreignLanguage: 'la fleur',
      localLanguage: 'flower',
      audio: 'audio/lo7/exercises/01-memory/006-la-fleur.mp3',
    },
    {
      foreignLanguage: 'la forêt',
      localLanguage: 'forest',
      audio: 'audio/lo7/exercises/01-memory/007-la-foret.mp3',
    },
    {
      foreignLanguage: 'l\'arbre',
      localLanguage: 'tree',
      audio: 'audio/lo7/exercises/01-memory/004-l-arbre.mp3',
    },
    {
      foreignLanguage: 'le babyfoot',
      localLanguage: 'table football',
      audio: 'audio/lo7/exercises/01-memory/010-le-babyfoot.mp3',
    },
    {
      foreignLanguage: 'le rugby',
      localLanguage: 'rugby',
      audio: 'audio/lo7/exercises/01-memory/001-le-rugby.mp3',
    },
    {
      foreignLanguage: 'le ski',
      localLanguage: 'skiing',
      audio: 'audio/lo7/exercises/01-memory/003-le-ski.mp3',
    },
  ],
...
const DICTATION_CONFIG = {
  ...
  phrases: [
    ['', '[Bonjour !]', 'audio/lo1/dialogues/01/001-salut-ca-va.mp3'],
    ['', '[Ça va, merci !]', 'audio/lo1/dialogues/02/003-ca-va-merci.mp3'],
    ['', '[Je m\'appelle Camille.]', 'audio/lo1/dialogues/03/003-je-mappelle-camille.mp3'],
  ],
...
const INLINE_CHOICE_CONFIG = {
  ...
  shuffleItems: false,
  items: [
    { text: '[*tu|tout]', audio: 'audio/lo1/exercises/01-word-parts/001-bonjour-mon-cheri.mp3' },
    { text: '[*nu|nous]', audio: 'audio/lo1/exercises/01-word-parts/002-bonsoir-mon-amour.mp3' },
    { text: '[rue|*roue]', audio: 'audio/lo1/exercises/01-word-parts/003-au-revoir-ma-cherie.mp3' },
  ],
...
const INLINE_TYPED_GAP_CONFIG = {
  ...
  items: [
    {
      prompt: 'What cheese do you like?',
      text: 'Qu\'est-ce que tu [aimes] comme [fromage] ?',
      audio: 'audio/lo1/dialogues/03/001-comment-tappelles-tu.mp3',
    },
    {
      prompt: 'What films do you like?',
      text: 'Qu\'est-ce que tu [aimes] comme [films] ?',
      audio: 'audio/lo1/dialogues/03/002-je-mappelle-max-et-toi.mp3',
    },
  ],
...
const PHRASE_REORDER_CONFIG = {
  ...
  phrases: [
    ['', 'homme', 'audio/lo1/exercises/07-listening-order/001-homme.mp3'],
    ['', 'hôtel', 'audio/lo1/exercises/07-listening-order/002-hotel.mp3'],
    ['', 'hôpital', 'audio/lo1/exercises/07-listening-order/003-hopital.mp3'],
    ['', 'horrible', 'audio/lo1/exercises/07-listening-order/004-horrible.mp3'],
  ],
...
const TYPED_TRANSFORM_CONFIG = {
  ...
  informationText: 'Listen and type the phone number you hear.',
  phrases: [
    ['', 'C\'est le [06 10 15 24]', 'audio/lo1/dialogues/01/001-salut-ca-va.mp3'],
    ['', 'C\'est le [07 12 18 36]', 'audio/lo1/dialogues/01/002-salut-ca-va-et-toi.mp3'],
  ],
...
const WORD_SPOT_CONFIG = {
  ...
  items: [
    {
      text: 'b[on]j[ou][r] m[on] ché[r]i',
      audio: 'audio/lo1/exercises/01-word-parts/001-bonjour-mon-cheri.mp3',
    },
    {
      text: 'b[on]s[oi][r] m[on] am[ou][r]',
      audio: 'audio/lo1/exercises/01-word-parts/002-bonsoir-mon-amour.mp3',
    },
    {
      text: 'au [r]ev[oi][r] ma ché[r]ie',
      audio: 'audio/lo1/exercises/01-word-parts/003-au-revoir-ma-cherie.mp3',
    },
  ],
...
const EXERCISES = [
  {
    ...
    description: 'Dropdown select per row — learner picks correct option from menu.',
  ...
  },
  {
    ...
    description: 'Drag word tiles into bracketed gaps within a passage.',
    render: () => <DraggableFillGaps config={DRAGGABLE_CONFIG} />,
  },
  {
    label: 'RadioQuiz',
    description: 'True/false (or multi-option) comprehension quiz with sequence audio.',
    render: () => <RadioQuiz config={RADIO_QUIZ_CONFIG} />,
  },
  {
    label: 'LineMatch',
    description: 'Draw lines between left-column items and right-column matches.',
  ...
  },
  {
    ...
    description: 'Type missing words inline within a sentence structure.',
  ...
  },
  {
    ...
    description: 'Listen to audio and type a transformed version (e.g. phone numbers).',
    render: () => <TypedTransformExercise config={TYPED_TRANSFORM_CONFIG} />,
  },
  {
    label: 'WordSpotExercise',
    description: 'Click on bracketed phonetic segments within words to identify sounds.',
    render: () => <WordSpotExercise config={WORD_SPOT_CONFIG} />,
  },
  {
    label: 'ClozeTypingExercise',
    description: 'Type missing words inline (gap-fill) — per-gap rather than global dictation.',
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="m-0 mt-1 text-sm text-muted-foreground">
              Dev-only page. Each section shows one exercise type with a minimal French fixture.
              Audio uses real clips from LO1/LO7 where available.
            </p>
          </div>
...
// ---------------------------------------------------------------------------
// Simple error boundary so one broken fixture doesn't crash the whole page
// ---------------------------------------------------------------------------
