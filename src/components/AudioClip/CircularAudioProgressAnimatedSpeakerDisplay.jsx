...
function CircularAudioProgressAnimatedSpeakerDisplayComponent(props) {
  const {
    handleClick,
    status = 'stopped',
    inline = false,
  ...
  // Mount + whenever progress/duration change: sync the stroke offset
  // imperatively (mirrors componentDidMount + componentDidUpdate).
  useEffect(() => {
  ...
  const requestedSize = Number.parseInt(props.size, 10);
...
export const CircularAudioProgressAnimatedSpeakerDisplay = memo(CircularAudioProgressAnimatedSpeakerDisplayComponent);
